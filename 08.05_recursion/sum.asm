...
; int sum(int)
SUMFN   ADD R6, R6, #-1     ; Push space for the return value.
        ...
        BRp SUMELSE         ; If "n <= 0", then:
        AND R0, R0, #0      ; ...set the return value to "0".
        STR R0, R5, #3
        BRnzp SUMRET        ; Else, do:
SUMELSE ADD R0, R0, #-1     ; ...compute "n - 1"...
        ADD R6, R6, #-1     ; ...push "n - 1"...
        STR R0, R6, #0
        JSR SUMFN           ; ...recurse...
        LDR R1, R6, #0      ; ...pop "sum(n - 1)"...
        ADD R6, R6, #1
        ADD R6, R6, #1      ; ...pop "n - 1"...
        LDR R0, R5, #4      ; ...load "n" into R0...
        ADD R0, R0, R1      ; ...compute "n + sum(n - 1)"...
        STR R0, R5, #3      ; ...set the return value to ^^.
